...
class ModelPusher:
    ...
    def initiate_model_pusher(self,)->artifact_entity.ModelPusherArtifact:
        try:
            #load 
            logging.info(f"loading input transformer, target transformer and model")
            ...
            utils.save_object(file_path = self.model_pusher_config.pusher_input_transformer_path, obj = input_transformer_object)
            utils.save_object(file_path = self.model_pusher_config.pusher_target_transformer_path, obj = target_transformer_object)
            ...
            utils.save_object(file_path = input_transfomer_path, obj = input_transformer_object)
            utils.save_object(file_path = target_transfomer_path, obj = target_transformer_object)
